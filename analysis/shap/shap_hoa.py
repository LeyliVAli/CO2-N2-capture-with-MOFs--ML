...
OUT=ROOT/"outputs"/"shap"/"hoa"
OUT.mkdir(parents=True,exist_ok=True)

MODEL_FILE=MODEL_DIR/"Final_Model_HOA.joblib"
...
imp_train.to_csv(OUT/"SHAP_Global_Train_HOA.csv",index=False)
imp_test.to_csv(OUT/"SHAP_Global_Test_HOA.csv",index=False)

shap.plots.bar(shap_test,max_display=TOP_N,show=False)
plt.tight_layout()
plt.savefig(OUT/"SHAP_Bar_HOA.png",dpi=400,bbox_inches="tight")
plt.close()

shap.plots.beeswarm(shap_test,max_display=TOP_N,show=False)
plt.tight_layout()
plt.savefig(OUT/"SHAP_Beeswarm_HOA.png",dpi=400,bbox_inches="tight")
plt.close()
...
for f in top_features:
    ...
    plt.tight_layout()
    plt.savefig(OUT/f"SHAP_Dependence_{f}_HOA.png",
...
pd.concat(rows).to_csv(
    OUT/"SHAP_Pressure_Resolved_HOA.csv",
    index=False
)
