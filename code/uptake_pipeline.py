...
def bootstrap_r2(y,p,g):
    ...
    for _ in range(N_BOOTSTRAP):
        ...
        idx=np.concatenate([
            rows[x] for x in sampled
        ])

        if np.unique(y[idx]).size>1:
            values.append(
                r2_score(y[idx],p[idx])
            )
...
if not np.isfinite(
    ...
    raise ValueError(
        "Non-finite values in training predictors."
    )
...
if not np.isfinite(
    ...
    raise ValueError(
        "Non-finite values in test predictors."
    )
...
joblib.dump(
    bundle,
    MODEL_DIR/"Final_Model_UP.joblib"
)
...
pd.DataFrame({
    ...
    "Absolute_Error":
        np.abs(yte.to_numpy()-pred_test)
}).to_csv(
    OUT/"Test_Predictions_UP.csv",
    index=False
)
...
summary=pd.DataFrame([{
    ...
    "Test_nMAE_IQR":
        test_metrics["MAE"]/iqr,

    "Test_nRMSE_IQR":
        test_metrics["RMSE"]/iqr,
