...
DATA=ROOT/"data"/"uptake"
MODEL_DIR=ROOT/"models"/"uptake"
OUT=ROOT/"outputs"/"shap"/"uptake"
OUT.mkdir(parents=True,exist_ok=True)

MODEL_FILE=MODEL_DIR/"Final_Model_UP.joblib"
...
idx=rng.choice(len(Xtrain),min(N_BACKGROUND,len(Xtrain)),replace=False)
...
imp_train.to_csv(OUT/"SHAP_Global_Train_UP.csv",index=False)
imp_test.to_csv(OUT/"SHAP_Global_Test_UP.csv",index=False)

shap.plots.bar(shap_test,max_display=TOP_N,show=False)
plt.tight_layout()
plt.savefig(OUT/"SHAP_Bar_UP.png",dpi=400,bbox_inches="tight")
plt.close()

shap.plots.beeswarm(shap_test,max_display=TOP_N,show=False)
plt.tight_layout()
plt.savefig(OUT/"SHAP_Beeswarm_UP.png",dpi=400,bbox_inches="tight")
plt.close()
...
for f in top_features:
    ...
    plt.tight_layout()
    plt.savefig(
        OUT/f"SHAP_Dependence_{f}_UP.png",
...
for p in sorted(pressure.unique()):
    ...
    imp=pd.DataFrame({
        "Feature":features,
        "Pressure_bar":p,
        "Mean_Abs_SHAP":np.abs(Mtrain[mask]).mean(axis=0)
    })
    rows.append(imp)

pd.concat(rows).to_csv(
    OUT/"SHAP_Pressure_Resolved_UP.csv",
    index=False
)
...
V=np.asarray(interaction_explainer.shap_interaction_values(Xi))
...
interactions.insert(0,"Rank",range(1,len(interactions)+1))
interactions.to_csv(
    OUT/"SHAP_Interactions_UP.csv",
    index=False
)
